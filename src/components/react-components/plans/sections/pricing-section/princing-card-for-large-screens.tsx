...
type PrincingCardForLargeScreensProps = {
  price: number;
  type: string;
  description: string;
  features: string[];
...
export function PrincingCardForLargeScreens({
  price,
  type,
  description,
  features,
  className,
  checkoutLink
}: PrincingCardForLargeScreensProps) {
  return (
    <motion.div
      className={clsx(
        'max-w-full w-full h-[659px] md:h-[728px] bg-grey-50 px-[32px] py-[32px] flex flex-col justify-between rounded-xlg',
        className
      )}
      initial="hidden"
      animate="visible"
    ...
    >
      <div className="flex flex-col">
        ...
        <motion.div className="mt-[17px]" initial="hidden" animate="visible" variants={FADE_IN}>
      ...
      <motion.a
        ...
        target="_blank"
        rel="noreferrer"
        className="w-full inline-block py-[12px] bg-[#848199] text-center rounded-md mb-[28px] hover:bg-[#928FAC] transition-colors"
        initial="hidden"
        animate="visible"
        variants={{
          visible: {
